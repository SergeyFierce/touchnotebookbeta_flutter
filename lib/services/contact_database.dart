...
class ContactDatabase {
  ...
  Future<List<Reminder>> _completeDueReminders({int? contactId}) async {
    final db = await database;
    final nowEpoch = DateTime.now().millisecondsSinceEpoch;
    final where = StringBuffer('completedAt IS NULL AND remindAt <= ?');
    final whereArgs = <Object?>[nowEpoch];
  ...
  Future<List<Reminder>> completeDueRemindersForContact(int contactId) async {
    return _completeDueReminders(contactId: contactId);
  }

  Future<List<Reminder>> completeAllDueReminders() async {
    return _completeDueReminders();
  }
